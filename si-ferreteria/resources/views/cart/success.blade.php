        ...
        <x-container-div class="container mx-auto px-4 max-w-4xl space-y-4">
            ...
            <x-container-second-div id="invoice-section">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <div>
                        ...
                        <div class="space-y-2 text-gray-400">
                            <p><span class="text-white font-semibold">Nombre:</span> {{ $order['customer']['name'] }}
                            </p>
                            <p><span class="text-white font-semibold">Email:</span> {{ $order['customer']['email'] }}
                            </p>
                            <p><span class="text-white font-semibold">Teléfono:</span>
                                {{ $order['customer']['phone'] ?? 'No proporcionado' }}</p>
                            <p><span class="text-white font-semibold">NIT/CI:</span>
                ...
                <div class="mb-6 pb-6 border-b border-gray-700">
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        @elseif(str_contains(strtolower($paymentMethodName), 'transferencia') ||
                                str_contains(strtolower($paymentMethodName), 'transfer'))
                            <span class="text-2xl">🏦</span>
                            <div>
                                <p class="text-white font-semibold">{{ $paymentMethodName }}</p>
                        ...
                        @foreach ($order['items'] as $id => $item)
                            <x-container-second-div class="flex items-center gap-4 p-4">
                                ...
                                <div class="flex-1">
                                    ...
                                    <p class="text-gray-400 text-sm">Precio unitario: USD
                ...
                <x-container-second-div class="bg-gray-700 rounded-lg p-6">
                    <div class="space-y-3">
                            ...
                            <div class="flex justify-between text-green-400">
                                <span>Descuento cupón:
                                    @if (isset($order['discount_code']))
                                        <span class="text-sm">({{ $order['discount_code'] }})</span>
                                    @endif
                                </span>
                            ...
                            <div class="flex justify-between text-orange-400">
                                <span>Descuento puntos:
                                    @if (isset($order['loyalty_points_used']))
                                        <span class="text-sm">({{ $order['loyalty_points_used'] }} puntos)</span>
                                    @endif
                                </span>
                                <span class="font-semibold">-USD
                                    {{ number_format($order['loyalty_discount'], 2) }}</span>
                            </div>
                        @endif
                        ...
                        <div class="border-t border-gray-600 pt-3 flex justify-between">
                            ...
                            <span class="text-3xl font-bold text-yellow-500">USD
                ...
                <ul class="space-y-2 text-gray-300">
                    @if (isset($order['customer']['phone']) && $order['customer']['phone'])
                        <li class="flex items-start gap-2">
                            ...
                            <span>Te contactaremos al {{ $order['customer']['phone'] }} para coordinar la
                                entrega</span>
                        </li>
                    @endif
                    <li class="flex items-start gap-2">
                    ...
                    @if (str_contains(strtolower($order['payment_method']), 'transferencia') ||
                            str_contains(strtolower($order['payment_method']), 'transfer'))
                        <li class="flex items-start gap-2">
                            <span class="text-yellow-500">✓</span>
            ...
            <div class="flex flex-col sm:flex-row gap-4 justify-center">
                <a href="{{ route('products.index') }}"
                    class="bg-blue-600 hover:bg-blue-700 text-white font-semibold px-8 py-3 rounded-lg transition text-center">
                    Seguir Comprando
                </a>
            ...
            <div class="text-center mt-8 text-gray-400">
                ...
                <p class="text-white font-semibold">Contáctanos: <a href="tel:+59160962433"
                        class="text-yellow-500 hover:underline">+591 609 624 33</a></p>
            </div>
        </x-container-div>
    </x-container-div>
